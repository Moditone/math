...
#ifndef MODITONE_MATH_CONSTANTS_HPP
#define MODITONE_MATH_CONSTANTS_HPP

namespace math
{
    template <class T> const T PI = T(3.1415926535897932384626433832795); //!< The ubiquitous π
    template <class T> const T TWO_PI = PI<T> * T(2); //!< 2 * π
    template <class T> const T HALF_PI = PI<T> / T(2); //!< Half π
    template <class T> const T QUARTER_PI = PI<T> / T(4); //!< A quarter π
